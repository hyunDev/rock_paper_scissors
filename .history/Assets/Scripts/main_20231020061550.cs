...
public class main : MonoBehaviour
{
    ...
    public Animation objUser;
    public Animation objCom;
    ...
    public void startGame(int choice) {
        ...
        // com 선택 animation 실행
        objCom.startComChoiceAnimation();
        // user, com 선택 결과 이미지 표시
        /* setComImage(com);
        objUser.GetComponent<Image>().sprite = Resources.Load<Sprite>("Image_" + choice) as Sprite; */
        /* if (com == 1) {
            comChoiceController.startScissorAnimation();
        } else if (com == 2) {
            comChoiceController.startRockAnimation();
        } else {
            comChoiceController.startPaperAnimation();
        } */
    ...
    public void choiceScissors() {
        // objUser = "가위";
        objUser.startScissorAnimation();
        startGame(1);
    }
    // 바위 선택
    public void choiceRock() {
        // objUser = "바위";
        objUser.startRockAnimation();
        startGame(2);
    }
    // 보 선택
    public void choicePaper() {
        // objUser = "보";
        objUser.startPaperAnimation();
        startGame(3);
    }
}
